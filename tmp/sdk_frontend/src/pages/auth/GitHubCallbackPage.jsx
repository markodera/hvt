...
import { refresh, socialAuthGithub } from '@/api/auth';
import { useAuth } from '@/hooks/useAuth';
import { HVTApiError } from '@/lib/hvt';
import {
                    ...
                    {!error ? (
                        <>
                            ...
                            <p className="mt-3 text-sm leading-7 text-[#a1a1aa]">
                                We are refreshing your session and waiting for the dashboard cookies to settle.
                            </p>
                        ...
                        <>
                            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border border-[#3f3f46] bg-[#18181b]">
        ...
        async function finishSignIn() {
            ...
            try {
                ...
                try {
                    ...
                    // Refresh re-sets cookies when available, but the callback keeps retrying
                    // session resolution even if refresh itself lands before cookies settle.
                }

                const me = await waitForSession({ attempts: 3, delayMs: 500 });
                ...
                navigate('/dashboard', { replace: true, state: { resolvedUser: me } });
            } catch (sdkError) {
                clearPendingSocialAuth();
                logSocialAuthTelemetry({
                    selectedProvider: SOCIAL_AUTH_PROVIDERS.GITHUB,
                    callbackProvider: SOCIAL_AUTH_PROVIDERS.GITHUB,
                    endpointHit: SOCIAL_AUTH_ENDPOINTS[SOCIAL_AUTH_PROVIDERS.GITHUB],
                    httpStatus: sdkError?.status ?? sdkError?.response?.status ?? null,
                    errorDetail: sdkError instanceof HVTApiError ? sdkError.detail : getErrorMessage(sdkError),
                    stage: 'callback_failed',
                });

                if (!cancelled) {
                    setError(getErrorMessage(sdkError));
                }
            }
        }
