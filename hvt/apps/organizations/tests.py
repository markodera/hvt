...
class APIKeyAuthenticationTestLiveTest(TestCase):
    ...
    def setUp(self):
        ...
        # Create test key
        test_prefix, self.test_full_key, test_hashed_key = APIKey.generate_key(environment="test")
        ...
        # Create live key
        live_prefix, self.live_full_key, live_hashed_key = APIKey.generate_key(environment="live")
    ...
    def test_authenticate_with_test_key(self):
        ...
        request = self.factory.get(
            '/api/test/',
            HTTP_X_API_KEY=self.test_full_key
        )
    ...
    def test_authenticate_with_live_key(self):
        ...
        request = self.factory.get(
            '/api/test/',
            HTTP_X_API_KEY=self.live_full_key
        )
    ...
    def test_reject_invalid_prefix_format(self):
        ...
        request = self.factory.get(
            '/api/test/',
            HTTP_X_API_KEY="hvt_invalid_abc123"
        )
