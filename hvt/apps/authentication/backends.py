...
class APIKeyAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        logger.info(f"[APIKeyAuthentication] Key received: {api_key[:20]}...")
    ...
    def validate_key(self, key: str):
        ...
        if not (key.startswith("hvt_test_") or key.startswith("hvt_live_")):
            ...
            raise exceptions.AuthenticationFailed("Invalid API key format. Expected hvt_test_* or hvt_live_*")
        ...
        logger.info(f"[APIKeyAuthentication] Looking for prefix: {prefix}, env: {environment}")
