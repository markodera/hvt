...
class IsAuthenticatedOrAPIKey(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        logger.info(f"[IsAuthenticatedOrAPIKey] request.auth type: {type(request.auth)}")
...
class IsAdminOrAPIKey(permissions.BasePermission):
    """
    Allow access if user is staff/Admin or APIKey is valid
    """
    def has_permission(self, request, view):
        ...
        if isinstance(request.auth, APIKey):
            logger.info("[IsAdminOrAPIKey] API Key detected, granting access")
            return True
...
class IsOrgAdminOrAPIKey(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        # API key always allowed
        if isinstance(request.auth, APIKey):
            logger.info("[IsOrgAdminOrAPIKey] API key detected, granting access")
            return True
            
        if request.user and request.user.is_authenticated:
            # Check if user belongs to an organization
            if not request.user.organization:
                logger.info(f"[IsOrgAdminOrAPIKey] User {request.user.email} has no organization")
                return False
                
            if request.user.can_manage_users():
                logger.info(f"[IsOrgAdminOrAPIKey] User {request.user.email} is {request.user.role}, granting access")
                return True
            logger.info(f"[IsOrgAdminOrAPIKey] User {request.user.email} is {request.user.role}, denying access")

        return False

class IsOrgOwnerOrAPIKey(permissions.BasePermission):
    """
    Allow access if user is org owner OR valid API key.
    Use for: API key management, org settings, role assignments.
    """
    ...
    def has_permission(self, request, view):
        # API key always allowed
        if isinstance(request.auth, APIKey):
            logger.info("[IsOrgOwnerOrAPIKey] API key detected, granting access")
            return True 
        ...
        if request.user and request.user.is_authenticated:
            # Check if user belongs to an organization
            if not request.user.organization:
                logger.info(f"[IsOrgOwnerOrAPIKey] User {request.user.email} has no organization")
                return False
                
            if request.user.is_org_owner():
                logger.info(f"[IsOrgOwnerOrAPIKey] User {request.user.email} is owner, granting access")
                return True
            logger.info(f"[IsOrgOwnerOrAPIKey] User {request.user.email} is {request.user.role}, denying access")
...
class IsOrgMemberOrAPIKey(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        # API key always allowed
        if isinstance(request.auth, APIKey):
            logger.info("[IsOrgMemberOrAPIKey] API key detected, granting access")
            return True
        
        # Check if user is authenticated and belongs to an org
        if request.user and request.user.is_authenticated:
            if request.user.organization:
                logger.info(f"[IsOrgMemberOrAPIKey] User {request.user.email} is org member, granting access")
                return True
            logger.info(f"[IsOrgMemberOrAPIKey] User {request.user.email} has no organization")
...
class IsSelfOrOrgAdmin(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.user and request.user.is_authenticated:
            ...
            if obj == request.user:
                logger.info(f"[IsSelfOrOrgAdmin] User {request.user.email} editing self")
                return True
            
            # Org admins can edit other users in their org
            if request.user.can_manage_users() and obj.organization == request.user.organization:
                logger.info(f"[IsSelfOrOrgAdmin] Admin {request.user.email} editing {obj.email}")
                return True
...
class CanChangeRole(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if obj == request.user and request.user.is_org_owner() and new_role != "owner":
            logger.warning(f"[CanChangeRole] Owner {request.user.email} cannot demote themselves")
            return False
